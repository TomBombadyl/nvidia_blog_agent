...
def test_ask(question: str = "What did NVIDIA say about RAG on GPUs?", top_k: int = 8) -> bool:
    ...
    try:
        ...
        if data.get('sources'):
            ...
            for i, source in enumerate(data['sources'][:3], 1):
                print(f"  {i}. {source.get('title', 'N/A')} (score: {source.get('score', 0):.4f})")
        
        return True
    except httpx.HTTPStatusError as e:
        print(f"❌ Ask endpoint failed with HTTP {e.response.status_code}: {e.response.text}")
        return False
    except Exception as e:
...
def test_ingest(api_key: str | None = None) -> bool:
    ...
    try:
        ...
        print(f"✅ Ingest endpoint passed!")
        ...
        if e.response.status_code == 401:
            print(f"❌ Ingest endpoint requires API key (401 Unauthorized)")
            print("   Set INGEST_API_KEY env var and pass it to this script")
        else:
            print(f"❌ Ingest endpoint failed with HTTP {e.response.status_code}: {e.response.text}")
        return False
    except Exception as e:
