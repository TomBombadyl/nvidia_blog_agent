...
async def main():
    ...
    try:
        ...
        if not config.rag.use_vertex_rag:
            logger.warning("⚠️  USE_VERTEX_RAG is not set to 'true'. This script is designed for Vertex RAG evaluation.")
            logger.warning("   Continuing anyway, but results may not reflect Vertex RAG performance.")
        
        logger.info(f"Using Gemini model: {config.gemini.model_name}")
        logger.info(f"Using RAG backend: {'Vertex AI' if config.rag.use_vertex_rag else 'HTTP'}")
        ...
        if args.cases_file:
            ...
            logger.info("Using default eval cases. Use --cases-file to provide custom cases.")
        ...
        for i, result in enumerate(results, 1):
            ...
            if result.retrieved_docs:
                print(f"    Top doc: {result.retrieved_docs[0].title} (score: {result.retrieved_docs[0].score:.4f})")
        ...
        output_data = {
            ...
            "results": [
                {
                    "question": r.question,
                    "expected_substrings": eval_cases[i].expected_substrings if i < len(eval_cases) else [],
