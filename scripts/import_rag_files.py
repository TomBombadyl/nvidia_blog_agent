...
async def wait_for_operation(client, operation_name, headers, location, project_id, max_wait_minutes=30):
...
async def import_rag_files_from_gcs():
    ...
    logger.info(f"📦 Will import in {len(batches)} batches (max {BATCH_SIZE} files per batch)")
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        for batch_idx, batch_uris in enumerate(batches, 1):
            logger.info("")
            logger.info(f"🚀 Importing batch {batch_idx}/{len(batches)} ({len(batch_uris)} files)...")
            ...
            for attempt in range(max_retries):
                ...
                if response.status_code == 200:
                    ...
                    if "name" in result:
                        ...
                        logger.info(f"   ✅ Batch {batch_idx} operation started: {operation_name}")
                        logger.info(f"   ⏳ Waiting for batch {batch_idx} to complete...")
                        
                        # Wait for operation to complete
                        await wait_for_operation(client, operation_name, headers, location, project_id)
                    ...
                    try:
                        ...
                        if "other operations running" in error_msg:
                            if attempt < max_retries - 1:
                                logger.info(f"   ⏳ Another operation is running. Waiting {retry_delay}s (attempt {attempt + 1}/{max_retries})...")
                                await asyncio.sleep(retry_delay)
                                continue
                            else:
                                logger.error(f"   ❌ Operation still running after {max_retries} attempts")
                                logger.error(f"   Please wait and run the script again later")
                                return False
                    except:
                        pass
                ...
                logger.error(f"❌ API Error ({response.status_code}) for batch {batch_idx}: {error_body}")
    ...
    logger.info(f"   Check status: https://console.cloud.google.com/ai/rag?project={project_id}")
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            project_id = os.environ.get("GOOGLE_CLOUD_PROJECT") or os.environ.get("GCP_PROJECT")
            logger.info(f"   https://console.cloud.google.com/ai/rag?project={project_id}")
        else:
