...
class AskResponse(BaseModel):
    ...
    session_id: Optional[str] = Field(default=None, description="Session ID for this conversation")
...
class BatchAskRequest(BaseModel):
    ...
    top_k: int = Field(default=5, ge=1, le=20, description="Number of documents to retrieve per question")
    ...
    try:
        ...
        async def check_rag_backend():
            ...
            try:
                ...
                test_docs = await retrieve_client.retrieve("test", k=1)
        ...
        async def check_qa_agent():
            ...
            return _qa_agent is not None, "QA agent initialized" if _qa_agent else "QA agent not initialized"
...
async def ask_question(request: Request, ask_request: AskRequest):
    ...
    try:
        ...
        if ask_request.use_cache:
            ...
            if cached_response:
                cached = True
                logger.info("Serving from cache", question_preview=ask_request.question[:100])
                return AskResponse(
...
async def ask_batch(request: Request, batch_request: BatchAskRequest):
    ...
    try:
        logger.info("Processing batch request", question_count=len(batch_request.questions))
        ...
        for (answer, docs), question in zip(results, batch_request.questions):
            ...
            batch_results.append(AskResponse(
                answer=answer,
                sources=sources,
                session_id=None,
                cached=False
        ...
        logger.info("Batch request completed", question_count=len(batch_request.questions))
...
async def export_data(
    ...
    if format.lower() == "csv":
        ...
        writer.writerow(["timestamp", "question", "answer", "sources_count", "latency_ms"])
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=query_history_{session_id or 'all'}.csv"}
        )
    else:
        return Response(
            content=json.dumps([
            ...
            headers={"Content-Disposition": f"attachment; filename=query_history_{session_id or 'all'}.json"}
        )
