...
def load_state_from_gcs(bucket_name: str, blob_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise IOError(f"Failed to parse JSON from gs://{bucket_name}/{blob_name}: {e}") from e
    except Exception as e:
        raise IOError(f"Failed to read state from gs://{bucket_name}/{blob_name}: {e}") from e
...
def save_state_to_gcs(
    ...
    try:
        ...
        raise IOError(f"Failed to write state to gs://{bucket_name}/{blob_name}: {e}") from e
...
def load_state(state_path: str | None = None) -> dict[str, Any]:
    ...
    if state_path.startswith("gs://"):
        ...
        if len(path_parts) != 2:
            raise ValueError(f"Invalid GCS URI format: {state_path}. Expected gs://bucket/blob")
