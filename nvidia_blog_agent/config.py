...
def load_config_from_env() -> AppConfig:
    ...
    if use_vertex_rag:
        ...
        if not rag_uuid:
            raise KeyError("RAG_CORPUS_ID or RAG_UUID environment variable is required for Vertex AI RAG")
        if not vertex_location:
            raise KeyError("VERTEX_LOCATION environment variable is required for Vertex AI RAG")
        if not docs_bucket:
            raise KeyError("RAG_DOCS_BUCKET environment variable is required for Vertex AI RAG")
        ...
        if not rag_base_url:
            raise KeyError("RAG_BASE_URL environment variable is required for HTTP-based RAG")
        if not rag_uuid:
            raise KeyError("RAG_UUID environment variable is required for HTTP-based RAG")
