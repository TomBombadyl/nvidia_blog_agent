...
class BlogPost(BaseModel):
    ...
    id: str = Field(..., description="Stable identifier, typically a hash of the URL")
...
class RawBlogContent(BaseModel):
    ...
    title: str = Field(..., description="Title extracted from the HTML")
    html: str = Field(..., description="Raw HTML content of the blog post")
    text: str = Field(
...
class BlogSummary(BaseModel):
    ...
    technical_summary: str = Field(
        ..., description="Detailed technical summary (2-5 paragraphs)", min_length=50
    )
    bullet_points: List[str] = Field(
        default_factory=list, description="Key takeaways in bullet point format"
    )
    ...
    def to_rag_document(self) -> str:
        ...
        if self.bullet_points:
            parts.extend(
                ["", "Key Points:", *[f"• {point}" for point in self.bullet_points]]
            )
        ...
        if self.keywords:
            ...
            categories = [
                kw for kw in self.keywords if "/" in kw or len(kw.split()) > 2
            ]
            other_keywords = [kw for kw in self.keywords if kw not in categories]

            if categories:
                parts.extend(["", f"Categories: {', '.join(categories)}"])
            if other_keywords:
                parts.extend(["", f"Keywords: {', '.join(other_keywords)}"])
...
class RetrievedDoc(BaseModel):
    ...
    blog_id: str = Field(..., description="Reference to the BlogPost.id")
    title: str = Field(..., description="Title of the retrieved blog post")
    url: HttpUrl = Field(..., description="URL of the source blog post")
    snippet: str = Field(
        ..., description="Relevant text snippet from the document", min_length=1
    )
    score: float = Field(
        ..., description="Relevance score from the retrieval system", ge=0.0, le=1.0
    )
