...
def _extract_post_from_element(
    ...
    if parent:
        ...
        category_heading = parent.find(["h2", "h3", "h4", "span"], class_=lambda x: x and ("category" in x.lower() or "tag" in x.lower() if x else False))
        if category_heading:
            ...
            try:
                ...
                if is_atom or not is_rss:
                    ...
                    for link in link_elems:
                        ...
                        if rel == 'alternate' or link.get('type') == 'text/html' or not link.get('rel'):
                            url = link.get('href')
                            if url:
                                break
                else:
                        ...
                        if content_type in ('html', 'xhtml', 'text/html'):
                            ...
                            if content_type == 'xhtml' and not content_text:
                                ...
                                for child in content_elem:
                                    xhtml_parts.append(ET.tostring(child, encoding='unicode', method='html'))
                                if xhtml_parts:
...
def discover_posts_from_feed(
    ...
    *,
    ...
    if (raw_feed_stripped.startswith('<?xml') or 
        raw_feed_stripped.startswith('<feed') or 
        raw_feed_stripped.startswith('<rss') or
        '<feed' in raw_feed_stripped[:100] or
        '<rss' in raw_feed_stripped[:100]):
